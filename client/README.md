# [ngJogging](http://jugalkishor-sujal.github.com/ng-Jogging) [![Build Status](https://api.travis-ci.org/jugalkishor-sujal/demo-MEAN.png?branch=v0.3.2-release)](https://travis-ci.org/jugalkishor-sujal/demo-MEAN)
...
```sh
$ git clone git://github.com/jugalkishor-sujal/ng-Jogging
$ cd ng-Jogging
...
Finally, open `file:///path/to/ng-Jogging/build/index.html` in your browser.

Happy hacking!

## Purpose

`ngJogging` is designed to make life easy by providing a basic framework
...
ngJogging also comes prepackaged with the most popular design frameworks
...
The principal goal of `ngJogging` is to set projects up for long-term
success.  So `ngJogging` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `ngJogging`.

But `ngJogging` is not an example of an AngularJS app: this is a
...
getting `ngJogging` up and running. Though `ngJogging` is really simple
...
`ngJogging` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/jugalkishor-sujal/ng-Jogging my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `ngJogging`, like Twitter Bootstrap
...
Technically, `ngJogging` is now ready to go.
...
`ngJogging`. What follows in this section is a quick introduction to the
...
`ngJogging` also includes [Live Reload](http://livereload.com/), so you no
...
default with `ngJogging`) you need to tell Live Reload to allow it. Go to
...
See the [issues list](http://github.com/jugalkishor-sujal/ng-Jogging/issues). And
