# The Tools Used in `ngJogging`

## Introduction

`ngJogging` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ngJogging` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ngJogging` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ngJogging`, this project makes the assumption
...
While `ngJogging` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ngJogging` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `ngJogging`; Grunt is
...
to get started with `ngJogging`. But for those curious or looking to go a
...
Grunt is the engine behind `ngJogging`. It's the magic that makes it move.
...
`ngJogging` comes with a `bower.json` file that looks something like this:
...
in `ngJogging` by default in there to see what I mean.
